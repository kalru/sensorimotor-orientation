...
class GridCellLocationRegion(PyRegion):
    ...
    def __init__(self,
        ...
        #rotation steps tracking
        self.steps_rotated = None
    ...
    def getOrientations(self):
        """Returns orientations of underlying modules in this region

        Returns:
            list: List of orientations in order
        """
        return [m.orientation for m in self._modules]

    def getStepsRotated(self):
        """Get the rotation of modules in this region inside this column.

        Returns:
            (int): Steps rotated
        """
        return self.steps_rotated

    def rotate_sensory_connections(self, steps=1, debug=False):
        """Rotate the sensory connections for the grid cell modules

        Args:
            steps (int, optional): The amount of steps to rotate. Defaults to 1.
            debug (bool, optional): Show verbose output. Defaults to False.

        Returns:
            (int): Placeholder
        """
        # need to cast it due to string language conversion
        steps = int(steps)
        # update internal steps
        self.steps_rotated += steps
        steps = steps * -1
        mods = [np.copy(i.A) for i in self._modules]
        # console.print(mods)
        idx = np.roll(np.array(range(len(mods))), steps)
        for i, mod in enumerate(self._modules):
            mod.A = mods[idx[i]]
        for i, mod in enumerate(self._modules):
            if np.degrees(mod.orientation) < np.degrees(self._modules[i+1].orientation):
                diff = np.degrees(self._modules[i+1].orientation) - np.degrees(mod.orientation)
                break
        # console.print([i.A for i in self._modules])
        # hard disable for now because somehow it hkeeps printing...
        if debug and 0:
            console.print("Shifted A matricies for modules by %d steps, which is %f degrees" % (steps, diff*steps))
        return 0

    def reset_sensory_connection_rotation(self):
        """Reset the rotation of grid cell modules by rotating them backwards.
        """
        self.rotate_sensory_connections(self.steps_rotated * -1, False)
